...
export default function NewsCard (){
  ...
  const router = useRouter()
    ...
    useEffect(() => {
          ...
          axios.get(apiUrl)// oyage numcll gnn ba data tynwa call wada na ne answer krnn 
            .then(response => {
   ...
   return(
    <div className="container mx-auto pb-6 px-8 ">
       ...
       {data.map((card) =>(
           ...
           } key={card._id} className="shadow-lg rounded-lg cursor-pointer">
                <img className="rounded-t-lg " src={card.image} alt=""/>
                <div className="p-5">
                    <div className=" ">
                        <h1 className="text-sm font-bold mb-3">{card.title}</h1>
                    <p className="text-xs text-justify">{card.content}</p>
                </div>
                </div>
